...
export default function SearchSelect({
    ...
    return (
        <div ref={ref} className="relative">
            ...
            <div
                onClick={() => setOpen((v) => !v)}
                className={`border rounded-lg w-full h-[40px] px-3 text-[14px] flex items-center justify-between cursor-pointer bg-surface transition-colors ${error ? "border-danger/50" : "border-border"
            ...
            {open && (
                <div className="absolute left-0 right-0 mt-1 bg-surface border border-border rounded-lg shadow-xl z-50 overflow-hidden flex flex-col">
                    <div className="relative p-2 border-b border-border-subtle">
                        <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-foreground-subtle" />
                        <input
                            autoFocus
                            value={query}
                            onChange={(e) => setQuery(e.target.value)}
                            placeholder="Qidirish..."
                            className="w-full h-8 pl-7 pr-2 text-[13px] rounded-md bg-surface-raised border border-transparent outline-none focus:border-primary text-foreground"
                        />
                    </div>
