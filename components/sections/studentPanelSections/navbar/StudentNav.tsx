...
export default function StudentNav({ onNavigate }: NavProps) {
  const { t } = useTranslation();
  const isActive = useActive();
  const { isSidebarCollapsed, setSidebarCollapsed, theme } = useUIStore();
  const collapsed = isSidebarCollapsed;

  const initiallyOpen = studentMenu
    ...
    >
      ...
      >
        ...
        <button
          ...
          title={collapsed ? t("student.nav.expand") : t("student.nav.collapse")}
        >
          <motion.div
