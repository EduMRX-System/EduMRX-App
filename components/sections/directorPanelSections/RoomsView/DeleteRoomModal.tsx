...
export default function DeleteRoomModal({ room, onClose, role = "director" }: Props) {
    ...
    return (
        <FormModalShell onClose={onClose} variant="center" maxWidth="max-w-md">
                <button type="button" onClick={onClose} className="absolute top-4 right-4 text-foreground-subtle hover:text-foreground cursor-pointer">
                    <X className="w-5 h-5" />
                </button>
                ...
                <div className="flex gap-4">
                    ...
                    <div>
                        <h3 className="text-[18px] font-semibold text-foreground">{t("director.rooms.delete.title")}</h3>
                ...
                <div className="mt-6 flex items-center justify-end gap-3">
                    ...
                    <button type="button" onClick={() => deleteRoom()} disabled={isPending} className="inline-flex items-center justify-center gap-2 h-10 px-5 bg-danger hover:bg-danger/90 text-white text-sm font-semibold rounded-lg disabled:opacity-60 cursor-pointer transition-colors">
