...
export default function TeacherNav({ onNavigate }: NavProps) {
  const { t } = useTranslation();
  const isActive = useActive();
  const { isSidebarCollapsed, setSidebarCollapsed, theme } = useUIStore();
  const collapsed = isSidebarCollapsed;

  const initiallyOpen = teacherMenu
    ...
    >
      ...
      >
        ...
        <button
          ...
          title={collapsed ? t("teacher.nav.expand") : t("teacher.nav.collapse")}
        >
          <motion.div
