...
import EditStudentModal from "@/components/sections/directorPanelSections/studentsView/EditStudentModal";
import AddStudentModal from "@/components/sections/directorPanelSections/studentsView/AddStudentModal";
    ...
    const { data: studentsData, isLoading, isError, error } = useQuery({
        ...
        queryFn: async () => {
            const res = await API.get<IStudentsResponse>(
                `director/students/?page=${page}&page_size=${pageSize}&search=${debouncedSearch}`
            );
            return res?.data;
        },
