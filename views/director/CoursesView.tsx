    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h1 className="text-xl font-semibold text-foreground">{t("director.courses.title")}</h1>
                    <p className="mt-0.5 text-sm text-foreground-muted">{t("director.courses.count", { count })}</p>
                </div>
                <button
            ...
            <div className="relative max-w-sm">
                ...
                <input
                    ...
                    placeholder={t("director.courses.search_placeholder")}
                    ...
                    <table className="w-full text-left text-sm">
                        <thead>
                            <tr className="border-b border-border text-xs uppercase tracking-wide text-foreground-muted  ">
                                <th className="px-5 py-3 font-medium">{t("director.courses.table.name")}</th>
                                <th className="px-5 py-3 font-medium">{t("director.courses.table.duration")}</th>
                                <th className="px-5 py-3 font-medium">{t("director.courses.table.price")}</th>
                                <th className="px-5 py-3 font-medium">{t("director.courses.table.status")}</th>
                                <th className="px-5 py-3 text-right font-medium">{t("director.courses.table.actions")}</th>
                            </tr>
                        </thead>
                            ...
                            {isLoading ? (
                                ...
                                <tr>
                                    <td colSpan={5} className="px-5 py-16 text-center">
                                        <BookOpen className="mx-auto mb-3 h-10 w-10 text-slate-300 dark:text-foreground-muted" />
                                        <p className="text-sm font-medium text-foreground-muted">{t("director.courses.empty.title")}</p>
                                        <p className="mt-1 text-sm text-foreground-subtle">{t("director.courses.empty.desc")}</p>
                                    </td>
                                </tr>
                            ) : (
                                courses.map((c) => (
